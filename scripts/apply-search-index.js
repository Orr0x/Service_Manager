...
async function runMigration() {
    ...
    try {
        ...
        // TARGET FILE: The global search indices migration
        const migrationFile = path.join(__dirname, '../supabase/migrations/20251210010000_add_global_search_indices.sql');
        const sql = fs.readFileSync(migrationFile, 'utf8');

        console.log('Running migration: 20251210010000_add_global_search_indices.sql ...');

        await client.query(sql);
        console.log('Migration completed successfully.');
    } catch (err) {
        console.error('Migration failed:', err.message);
        // Do not verify validation error if it's "column exists"
        if (err.message.includes('already exists')) {
            console.log('Migration apparently already run or partially run.');
        } else {
            process.exit(1);
        }
    } finally {
