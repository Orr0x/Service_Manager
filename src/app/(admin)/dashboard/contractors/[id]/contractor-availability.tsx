...
export function ContractorAvailability({ contractorId }: ContractorAvailabilityProps) {
    const [startDate, setStartDate] = useState('')
    const [endDate, setEndDate] = useState('')
    const [reason, setReason] = useState('')

    const { data: unavailability, refetch } = api.contractors.getUnavailability.useQuery({ contractorId })

    const addUnavailability = api.contractors.addUnavailability.useMutation({
        onSuccess: () => {
            setStartDate('')
            setEndDate('')
            setReason('')
    ...
    const removeUnavailability = api.contractors.removeUnavailability.useMutation({
        onSuccess: () => {
            refetch()
        },
    })
    ...
    return (
        <div className="space-y-6">
            <div>
                ...
                <p className="mt-1 text-sm text-gray-500">Manage days when this contractor is unavailable for scheduling.</p>
            </div>
